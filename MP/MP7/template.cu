...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (row < dim) {
      float num = 0;
      int sec = 0;
      while (sec < matRows[row]) {num += matData[matColStart[sec] + row] * vec[matCols[matColStart[sec] + row]]; sec++;}
      out[matRowPerm[row]] = num;
  }
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 GridDim(ceil(1. * dim * dim / BLOCK_SIZE), 1, 1);
