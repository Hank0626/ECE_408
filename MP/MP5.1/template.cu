...
__global__ void total(float *input, float *output, int len) {
  ...
  //@@ correct index
  __shared__ float ps[BLOCK_SIZE * 2];
  unsigned int t = threadIdx.x;
  unsigned int start = 2 * blockDim.x * blockIdx.x;
  
  if (start + t < len) ps[t] = input[start + t];
  else ps[t] = 0;

  if (start + blockDim.x + t < len) ps[blockDim.x + t] = input[start + blockDim.x + t];
  else ps[blockDim.x + t] = 0;
     
  for (unsigned int stride = blockDim.x; stride >= 1; stride >>= 1) {
    __syncthreads();
    if (t < stride) ps[t] += ps[t+stride];
  }
  if (t == 0) output[blockIdx.x] = ps[t];
}
...
int main(int argc, char **argv) {
  ...
  dim3 DimGrid(ceil(numInputElements / 2. / BLOCK_SIZE), 1, 1);
