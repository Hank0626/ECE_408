...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    const int Height_out = Height - K + 1, Width_out = Width - K + 1;
    ...
    int height = blockIdx.z / int(ceil(Width_out * 1. / TILE_WIDTH)) * TILE_WIDTH + threadIdx.y;
    int width = blockIdx.z % int(ceil(Width_out * 1. / TILE_WIDTH)) * TILE_WIDTH + threadIdx.x;


    if ((width < (Width_out)) && (height < (Height_out))) {
        float tol = 0.0f;
        for (int c = 0; c < Channel; c++)
            for (int m = 0; m < K; m++)
                for (int n = 0; n < K; n++)
                    tol += in_4d(bx, c, height + m, width + n) * mask_4d(by, c, m, n);
        out_4d(bx, by, height, width) = tol;
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 gridDim(Batch, Map_out, int(ceil(Width_out * 1. / TILE_WIDTH)) * int(ceil(Height_out * 1. / TILE_WIDTH)));
